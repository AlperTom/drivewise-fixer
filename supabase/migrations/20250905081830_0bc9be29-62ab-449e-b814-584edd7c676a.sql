-- SECURITY FIX: Essential customer data protection (corrected syntax)
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE id = check_company_id AND user_id = check_user_id
  ) THEN
    RETURN data_field;
  ELSE
...
-- Add security constraints (without IF NOT EXISTS)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.constraint_column_usage 
    WHERE table_name = 'appointments' AND constraint_name = 'check_company_required'
  ) THEN
    ALTER TABLE public.appointments ADD CONSTRAINT check_company_required 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.constraint_column_usage 
    WHERE table_name = 'leads' AND constraint_name = 'check_leads_company_required'
  ) THEN
    ALTER TABLE public.leads ADD CONSTRAINT check_leads_company_required 
      CHECK (company_id IS NOT NULL);
  END IF;
END $$;

-- Create security indexes
CREATE INDEX IF NOT EXISTS idx_appointments_security ON public.appointments(company_id, customer_email);
CREATE INDEX IF NOT EXISTS idx_leads_security ON public.leads(company_id, email);
...
GRANT EXECUTE ON FUNCTION public.log_customer_access(text, uuid, text, text, text) TO authenticated;